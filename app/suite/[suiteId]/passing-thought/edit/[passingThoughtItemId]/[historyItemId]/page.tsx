...
export default function PassingThoughtEditRoute({
  ...
  useEffect(() => {
    fetch(
      new Request(
        `/api/suite/${suiteId}/passing-thought/${passingThoughtItemId}/${historyItemId}`,
        {
          method: 'GET',
        },
      ),
  ...
  return (
    <>
      <ItemForm
        ...
        cancelUrl={`/suite/${suiteId}/passing-thought/view/${passingThoughtItemId}/${historyItemId}`}
        apiEndpoint={`/api/suite/${suiteId}/passing-thought/${passingThoughtItemId}/${historyItemId}`}
        apiMethod="PATCH"

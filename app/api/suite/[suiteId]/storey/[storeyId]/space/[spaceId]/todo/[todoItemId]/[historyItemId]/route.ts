...
export const GET = async (
  request: Request,
  { params: { storeyId, spaceId, todoItemId, historyItemId } }: TodoGetParams,
) => {
  ...
  return NextResponse.json({
    ...
    storey: existingTodoHistoryItem.storey
      ...
      : null,
    space: existingTodoHistoryItem.space
      ? {
          id: existingTodoHistoryItem.space.id,
          name: existingTodoHistoryItem.space.name,
          description: existingTodoHistoryItem.space.description,
        }
      : null,
    todoItem: {
...
export const PATCH = async (
  request: Request,
  { params: { storeyId, spaceId, todoItemId, historyItemId } }: TodoPatchParams,
) => {
  const user = await getUserAuth()
  ...
  return NextResponse.json({
    success: true,
    redirect: `/suite/${existingTodo.storey.suite.id}/storey/${storeyId}/space/${spaceId}/dashboard/todo/view/${newTodoItem.todoItem.model_id}/${newTodoItem.todoItem.history_id}`,
  })
}
